...
**A Dagger-backed per-conversation sandbox, not a reuse of the K8s-pod-per-MCP-server mechanism.** The platform already has an isolation primitive for running untrusted third-party code in its own process boundary — the Kubernetes Deployment each installed local MCP server gets (see [MCP Gateway & Orchestrator](./05-mcp-gateway-and-orchestrator.md#k8s-orchestrator-design-pod-per-server)). The sandbox doesn't reuse it, because the two are solving different-shaped problems. An installed MCP server exposes a **fixed, predefined tool surface** (specific functions with specific typed arguments) and is provisioned **once per install**, then shared across however many conversations use it — a K8s Deployment's cold-start cost (scheduling, image pull, readiness probes: seconds to tens of seconds) is acceptable to pay once. The skill sandbox instead needs to support **arbitrary, open-ended code the model writes on the spot**, materialized fresh on effectively every `run_command` call — a far higher-frequency operation than "install a server." Paying full K8s pod cold-start cost on every tool call would be untenable at interactive latency; Dagger's content-addressed layer cache is what makes "rebuild the container from the replay-log recipe on every call" cheap in the common case, because only the newly appended step is genuine new work — the unchanged prefix hits cache. *Cost*: this is a real, not eliminated, latency tradeoff — cache misses still happen (the first command in a brand-new sandbox, a Dagger engine restart evicting the cache, or a long conversation's replay chain approaching `MAX_REPLAY_FS_LAYERS`) and pay the slower rebuild path. The platform accepted "usually fast, occasionally slower, with the failure mode surfaced explicitly" (`ARCHESTRA_SANDBOX_HISTORY_LIMIT`) over either extreme: a persistent per-conversation container (resource cost of many idle warm containers, plus the state-and-reproducibility problems the replay design exists to avoid) or reusing the K8s-pod mechanism as-is (wrong cold-start budget for the call frequency).
